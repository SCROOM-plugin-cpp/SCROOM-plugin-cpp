...
void ColorConfig::loadFile(std::string file) {
  ...
  if (!boost::filesystem::exists(full_path)) { // File does not exist on file system
      std::cout << "WARNING: Colours file does not exist at path: " + full_path.string() + "\n";
  ...
  try {
    ...
    // Loading didnt work
    std::cout << "WARNING: Loading colours file failed. file at: " + full_path.string() + " is most likely ill formed.\n";
    std::cout << "Loading default CMYK \n";
    addNonExistentDefaultColors();
    return;
  }
...
void ColorConfig::parseColor(pt::ptree::value_type &v,
                             std::unordered_set<std::string> &seenNamesAndAliases){
