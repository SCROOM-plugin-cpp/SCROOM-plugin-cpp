...
SepFile SepSource::parseSep(const std::string &file_name) {
    ...
    while (std::getline(file, line)) {
        ...
        if (result[0] != "C" && result[0] != "M" && result[0] != "Y" &&
            ...
            // Unsupported channel
            warnings +=
                "WARNING: The .sep file defines an unknown channel (not C, "
                "M, Y, K, V or W)!\n";
            continue;
        }
...
void SepSource::openFiles() {
    for (auto c : channels) {
        if (channel_files[c] != nullptr) {
            printf(
                "WARNING: %s file has already been initialized. Cannot open it "
                "again.\n",
                c.c_str());
            return;
        }
    ...
    if (show_warning) {
        printf(
            "PANIC: One of the provided files is not valid, or could not be "
            "opened!\n");
        ShowWarning(
            "PANIC: One of the provided files is not valid, or could not "
            "be opened!");
    }
}
